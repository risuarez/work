...
public class ValidarseAction implements Accion
{
			...
			HttpServletResponse response)
	{
		...
		{
			Log.info("Campo/s vacio al intentar identificarse ");
			request.setAttribute("error", "Debe escribir usuario y contraseña");
			resultado = "FRACASO";
		} else
		{
			if (session.getAttribute("user") == null)
			{
				...
				if (userByLogin != null)
				{
					...
					{
						Log.info("Contraseña del usuario [%s] incorrecta",
								nombreUsuario);
			...
			} else if (!nombreUsuario.equals(session.getAttribute("user")))
			{
				...
				request.setAttribute("error",
						"Ha intentado iniciar una nueva sesión con una sesión ya abierta");
				session.invalidate();
				resultado = "FRACASO";
			}
		}
